...
export function astar(grid, startNode, finishNode) {
  // Init data.
  let queue = new BinaryHeap(function(x) { 
    return x.distance + Math.abs(finishNode.row - x.row) + Math.abs(finishNode.col - x.col) 
  })
  ...
  for(;;) {
    ...
    const neighbors = getNeighbors(element, algGrid);

  for (const neighbor of neighbors) {
    const newDistance = element.distance + neighbor.weight;
